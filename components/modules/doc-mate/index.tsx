...
const firstAidResponses = {
  ...
  cpr: "CPR steps: 1) Check if the person is responsive. 2) Call emergency services (108). 3) Place the person on their back on a firm surface. 4) Place your hands on the center of their chest. 5) Perform chest compressions at a rate of 100-120 per minute, pushing at least 2 inches deep. 6) If trained, give rescue breaths after 30 compressions. 7) Continue until help arrives. Remember, proper training is recommended for performing CPR effectively.",
...
export default function DocMate() {
  ...
  const getFirstAidResponse = (input: string) => {
    ...
    return "I don't have specific information about that situation. For any medical emergency, it's best to call emergency services (108) immediately. For non-emergencies, consult with a healthcare professional for proper advice."
  }
  ...
  const handleSend = async () => {
    ...
    if (isDefault) {
      fullResponse = `Gemini AI says: ${geminiResponse}\n\nIMPORTANT: This is general first aid guidance only. Always call emergency services (108) for serious situations. This information is not a substitute for professional medical advice or training.`;
    } else {
